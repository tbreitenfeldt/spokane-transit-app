...
function init () {
	...
	$("#clearResultsButton").click(clearScheduleArea);
}//end function
...
function outputScheduleInformation(data, routeMap, stopMap, outputRegionID) {
    ...
    if (scheduleInfo.length > 0) {
        let currentStopName = stopMap[scheduleInfo[0]["destination_onestop_id"]];
        text = currentStopName + "<br>";
...
function getAllRouts(event) {
    ...
    if (regex.test(code)) {
        ...
        setTimeout(function() {$("#errorRegion").html("");}, 5000);
    }//end else
}//end function 
...
function processInvalidStopCode(error) {
        $("#errorRegion").html("<p>Invalid code, that stop ID does not exist.");
        setTimeout(function() {$("#errorRegion").html("");}, 5000);
}//end function
...
function getRoutesFromCode(data) {
    ...
    //create route map of bus numbers to pass into populateRouteButtons
    for (let route of routes) {
        let routeID = route["route_onestop_id"];
        let busNumber = route["route_name"];
        routeMap[routeID] = busNumber;
...
function clearScheduleArea(){
	$('#routesForStopHeaderRegion').empty();
    $('#busButtonsRegion').empty();
	$('#busScheduleRegion').empty();
}
